...
class ArticleServiceTest {
    ...
    @DisplayName("[검색어 없이 검색] 검색어 없이 게시글을 검색하면, 게시글 리스트 페이지를 반환한다.")
    @Test
    ...
    @DisplayName("[검색어 함께 검색] 검색어와 함께 게시글을 검색하면, 게시글 페이지를 반환한다.")
    @Test
    void givenSearchParameters_whenSearchingArticles_thenReturnsArticlePage() {
        // Given
        SearchType searchType = SearchType.TITLE;
        String searchKeyword = "title";
        Pageable pageable = Pageable.ofSize(20);  // 페이지 객체를 요청하고, 페이지에 포한될 요소를 20개로 설정
    ...
    @DisplayName("[검색어 없이 해시태그 검색] 검색어 없이 게시글을 해시태그 검색하면, 빈 페이지를 반환한다.")
    @Test
    ...
    @DisplayName("[검색어와 함께 해시태그 검색] 검색어와 함께 해시태그를 검색하면, 게시글 페이지를 반환한다.")
    @Test
    ...
    void givenArticleId_whenSearchingArticle_thenReturnsArticle() {
        // Given
        Long articleId = 1L;
        Article article = createArticle();  //Article.of(createUserAccount(), "title", "content", "#java")
        ...
        // Then
        assertThat(dto)
                // dto 객체가 title 필드를 가지고 있고, 해당 필드의 값이 article 객체의 title 필드 값과 같은지를 확인
    ...
    @DisplayName("[해시태그 리스트 조회] 해시태그를 조회하면, 유니크 해시태그 리스트를 반환한다")
    @Test
    ...
    @DisplayName("[없는 게시글을 조회] 없는 게시글을 조회하면, 예외를 던진다.")
    @Test
    void givenNonexistentArticleId_whenSearchingArticle_thenThrowsException() {
        ...
        // When
        // catchThrowable 메서드는 주어진 람다 표현식을 실행하고, 해당 람다가 예외를 throw하는지 여부를 검증
        // 예외를 반환하고, 그렇지 않으면 null을 반환
    ...
    @DisplayName("[새 게시글 생성] 게시글 정보를 입력하면, 게시글을 생성한다")
    @Test
    void givenArticleInfo_whenSavingArticle_thenSavesArticle() {
        // Given
        ArticleDto dto = createArticleDto(); //createArticleDto("title", "content", "#java")
    ...
    @DisplayName("[게시글 수정] 게시글의 수정 정보를 입력하면, 게시글을 수정한다")
    @Test
