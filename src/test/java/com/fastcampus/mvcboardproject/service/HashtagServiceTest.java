...
class HashtagServiceTest {
    ...
    @Test
    @DisplayName("게시글이 남아있는 경우, 해시태그를 삭제하지 않는다.")
    void givenHashtagId_whenHavingArticles_thenDeletesHashtag() {
        // Given
        Long hashtagId = 1L;
        Hashtag hashtag = mock(Hashtag.class);                                      // Hashtag 모의 객체 생성
        Set<Article> articles = new LinkedHashSet<>();
        articles.add(mock(Article.class));                                          // 비어 있지 않은 Set<Article>

        given(hashtagRepository.getReferenceById(hashtagId)).willReturn(hashtag);
        given(hashtag.getArticles()).willReturn(articles);                          // 비어 있지 않은 Set 반환 설정
    ...
    @Test
    @DisplayName("게시글이 남아있지 않은 경우, 해시태그를 삭제한다.")
    void givenHashtagId_whenNotHavingArticles_thenDeletesHashtag() {
        // Given
        Long hashtagId = 1L;
        Hashtag hashtag = mock(Hashtag.class);
        Set<Article> articles = new LinkedHashSet<>();                              // 비어있는 Set<Article>

        given(hashtagRepository.getReferenceById(hashtagId)).willReturn(hashtag);
        given(hashtag.getArticles()).willReturn(articles);                          // 비어 있지 Set 반환 설정
